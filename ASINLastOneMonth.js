...
* of each review type within the last month (30 days)
* @param {string[]} param - Can be one or multiple ASINs
*/
function getReviewBreakdownLastOneMonth(...param) {
    ...
    // Dates are used in Epoch (UNIX) Time to measure up to 30 days prior
    const today = new Date();
    const todayEpochTime = Date.parse(today);
    const oneMonthsInMilliseconds = 2592000000;
    const oneMonthsPrior = new Date(todayEpochTime - oneMonthsInMilliseconds);
    const oneMonthsPriorEpochTime = Date.parse(oneMonthsPrior);
    ...
    (function selectReview() {
      for (let reviewRow of reviews) {
        ...
        if (currentReviewDate < oneMonthsPriorEpochTime) {
          break; // break out of loop if over 30 days prior
        }
